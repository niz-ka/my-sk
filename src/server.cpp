...
Server::Server(int _port) : socketFd(0), port(_port), address({}), pollfds({}), clients({}), games({})
{
}
...
void Server::disconnectClient(int clientFd, std::size_t i)
{
    printf("[INFO] Client with address %s and port %d disconnected. Number of clients: %zu\n",
           inet_ntoa(clients[clientFd].getAddressPointer()->sin_addr),
           clients[clientFd].getAddressPointer()->sin_port,
    ...
    if (games[gameCode].getOwnerSocket() == clientFd)
    {
        ...
        {
            if (client.first == clientFd || client.second.getGameCode() != gameCode)
                continue;
...
int Server::connectClient()
{
    ...
    int clientSocket = accept(socketFd, (sockaddr *)&remoteAddress, &size);
    ...
    clients[clientSocket] = Client(remoteAddress);
    printf("[INFO] %zu. client connected with ip: %s and port: %d\n",
           getNumberOfClients(),
           inet_ntoa(clients[clientSocket].getAddressPointer()->sin_addr),
           clients[clientSocket].getAddressPointer()->sin_port);
...
void Server::makeAction(std::string &message, const int clientFd)
{
    ...
    if (action == MESSAGE::QUESTION)
    {
        ...
        if (prefix == MESSAGE::QUESTION_START)
        {
            ...
            games[code] = Game(clientFd);
            // Przypisz temu klientowi grę o tym kodzie
            clients[clientFd].setGameCode(code);
        ...
        else
        {
            ...
            {
                games[gameCode].getQuestions().emplace_back();
                games[gameCode].getQuestions().back().setQuestion(message);
            }
            else if (prefix == MESSAGE::QUESTION_ANSWER_A)
            {
                games[gameCode].getQuestions().back().setAnswerA(message);
            }
            else if (prefix == MESSAGE::QUESTION_ANSWER_B)
            {
                games[gameCode].getQuestions().back().setAnswerB(message);
            }
            else if (prefix == MESSAGE::QUESTION_ANSWER_C)
            {
                games[gameCode].getQuestions().back().setAnswerC(message);
            }
            else if (prefix == MESSAGE::QUESTION_ANSWER_D)
            {
                games[gameCode].getQuestions().back().setAnswerD(message);
            }
            else if (prefix == MESSAGE::QUESTION_CORRECT)
            {
                games[gameCode].getQuestions().back().setCorrect(message);
            }
            else if (prefix == MESSAGE::QUESTION_TIME)
            {
                games[gameCode].getQuestions().back().setTime(message);
            }
            else
            {
    ...
    else if (action == MESSAGE::JOINING)
    {
        ...
        for (auto &game : games)
        {
            ...
            if (submittedCode == correctCode)
            {
                ...
                clients[clientFd].setGameCode(correctCode);
    ...
    else if (action == MESSAGE::NICK_CHOOSING)
    {
        ...
        if (games[gameCode].isStarted())
        {
            ...
            clients[clientFd].setGameCode(0);
            sendData(clientFd, msgToStr(MESSAGE::GAME_ALREADY_STARTED));
    ...
    else if (action == MESSAGE::GAME_START)
    {
        ...
        games[gameCode].setStarted(true);
        ...
        for (auto &client : clients)
        {
            if (client.first == clientFd || gameCode != client.second.getGameCode())
                continue;
            for (auto &anotherClient : clients)
            {
...
void Server::run()
{
    ...
    pollfds.push_back({socketFd, POLLIN, 0});
    ...
    while (true)
    {
        ...
        for (auto i = 0; i < pollfds_size; ++i)
        {
            ...
            else
            {
                ...
                int bytes = readData(clientFd, HEADER_LENGTH, messageLength);
                if (bytes == 0)
                {
                    disconnectClient(clientFd, i);
                    pollfds_size = pollfds.size();
                }
